...
class Message():
	def __init__(self,messageType,lamportTime,numLikes,senderInfo):
...
def readPost(filename):
	...
	likes = lines[1]
	print "Post : " + str(post)
	print "Initial Number of Likes : " + str(likes)
	return [post,likes]
...
class server_thread(threading.Thread):
	...
	def run(self):
		###print "I am in server and my name is " + self.name
	...
	def send_info(self):
		...
		while(True):
			if (not exitQueue.empty()):
				...
				ref_client_info['client1'].send(pickle.dumps(Message("QUIT",lamportTime,numLikes,self.name)))
				ref_client_info['client2'].send(pickle.dumps(Message("QUIT",lamportTime,numLikes,self.name)))
				ref_client_info['client3'].send(pickle.dumps(Message("QUIT",lamportTime,numLikes,self.name)))
				break
			while(not selfProcessQueue.empty()):
				##print "In self processing"
				temp = selfProcessQueue.get()
				#self.currLamportTime = self.currLamportTime + 1
				self.selfMessageQueue.put(Message("Request",self.currLamportTime,self.numLikes,self.name))
			...
			while(not self.selfMessageQueue.empty()):
				...
				totalOrderedQueue.put(Message("Request",self.currLamportTime,self.numLikes,self.name))
				## Sending the requests to all the process when Liked after a delay of 5s
				ref_client_info['client1'].send(pickle.dumps(Message("REQUEST",self.currLamportTime,self.numLikes,self.name)))
				print (self.name).upper()+ ": Request sent to " + name_info['client1'] + " Time : " + str(self.currLamportTime )
				ref_client_info['client2'].send(pickle.dumps(Message("REQUEST",self.currLamportTime,self.numLikes,self.name)))
				print (self.name).upper()+ ": Request sent to " + name_info['client2'] + " Time : " + str(self.currLamportTime )
				ref_client_info['client3'].send(pickle.dumps(Message("REQUEST",self.currLamportTime,self.numLikes,self.name)))
			...
			while(not requestQueue.empty()):
				...
				ref_client_info[client2name[incomingSender]].send(pickle.dumps(Message("REPLY",self.currLamportTime,self.numLikes,self.name)))
			...
			while((not releaseQueue.empty()) or (check_dict(self.replyDict))):
				...
				if((check_dict(self.replyDict))):
					##print "In checking the dict"
					##print totalOrderedMessage.getSenderInfo(),  self.name
					if((totalOrderedMessage.getSenderInfo() == self.name)):
						...
						ref_client_info['client1'].send(pickle.dumps(Message("RELEASE",self.currLamportTime,self.numLikes,self.name)))
						print (self.name).upper()+ ": Release sent to " + name_info['client1'] + " Time : " + str(self.currLamportTime) 
						self.currLamportTime += 1

						ref_client_info['client2'].send(pickle.dumps(Message("RELEASE",self.currLamportTime,self.numLikes,self.name)))
						print (self.name).upper()+ ": Release sent to " + name_info['client2'] + " Time : " + str(self.currLamportTime)
						self.currLamportTime += 1

						ref_client_info['client3'].send(pickle.dumps(Message("RELEASE",self.currLamportTime,self.numLikes,self.name)))
						...
						## Sleep because not to over ride the release messages sent to similiar client
						time.sleep(1)
...
class client_thread(threading.Thread):
	...
	def run(self):
		###print "I am in client and my name is " + self.name
		self.invoke_client()
		###self.reqQueue.put(self.name)
		self.get_info()
	...
	def invoke_client(self):
		...
		while (True):
			try:
				...
				continue
		###print "Sending the connection to server " + self.name
	...
	def get_info(self):
		while True:
			...
			if(recvdMessage.getMessageType() == "REPLY"):
				### print name_info['server'].upper() + ": Reply has been received from " + recvdMessage.getSenderInfo()
				self.replyQueueLock.acquire()
			...
			if(recvdMessage.getMessageType() == "RELEASE"):
				### print name_info['server'].upper() + ": Release has been received from " + recvdMessage.getSenderInfo()
				self.releaseQueueLock.acquire()
...
def process(argv):
	...
	###print "Invoked necessary threads"
